program TrajektorieKoerperGleicherMassen


!Die Version dient, zur Ueberpruefung des Programm Teilchenbahn. Da die Vermutung bestand, 
!dass die Gleichungen fuer die Beschleunigung a_x, a_y, a_z falsch seien konneten habe ich, 
!in dieser Version das urspruengliche Programm genommen und vereinfacht, indem ich die Massen 
!von Saturn und Rhea gleichgross gemacht habe, sodass sich der Lagrangepunkt genau in der 
!Mitte befindet. Desweiteren habe ich beide Massekoerper nicht mehr umeinander kreisen lassen, 
!sodass der Lagrangepunkt auch raeumlich an der selben Stelle bleiben sollte.    
  ...
  dt   = 2.0                ![s]
  n    = 500                 ![Anzahl der Rechnungen]
  t = 0.0 
 


  !STARTWERTE IM RUHESYSTEM DES SATURN:   
  xs = 1000         ![m]  521203182.7
  ys = 0.0          !     0.0
  zs = 0.0          !     1528000.0
  ...
  xr = 1000 !5836817.3      !Teilchen befinde sich bei t=0s genau am Nordpol
  yr = 0.0                             !0.0                
  zr = 0.0                             !1528000.0  !Radius Rhea r= 1528000m                             
  v_xr = 0.0               !Geschwindigkeit des Teilchen bei t= 0s (Sputtering)
  v_yr = 0.0
  v_zr = 0.0 
  ...
  !W(7) = -(gamma * Ms * xs)/(xs**2 + ys**2 + zs**2)**(1.5) - (gamma * Mr * xr)/(xr**2 + yr**2 + zr**2)**(1.5) &
  !     + xr * omegaz**2 + 2 * omegaz * v_yr   
  !W(8) = -(gamma * Ms * ys)/(xs**2 + ys**2 + zs**2)**(1.5) - (gamma * Mr * yr)/(xr**2 + yr**2 + zr**2)**(1.5) &
  !     + yr * omegaz**2 - 2 * omegaz * v_xr  
  !W(9) = -(gamma * Ms * zs)/(xs**2 + ys**2 + zs**2)**(1.5) - (gamma * Mr * zr)/(xs**2 + ys**2 + zs**2)**(1.5)     
  ...
  write(*,*) "   t        x[m]           y[m]      z[m]    v_x[m/s]   v_y[m/s]  v_z[m/s]"
...
end program TrajektorieKoerperGleicherMassen
